...
class FirebaseService {
  ...
  static Future<void> insertData(TaskModel model) async {
    String str = auth.currentUser!.email.toString().replaceAll(".", "1");
  ...
  static Future<void> createAccount() async {
    try {
      ...
      final String str = signUpController.email.value.text.toString().replaceAll(".", "1");
      ...
      googleSignIn
        ...
        if (googleSignInAccount != null) {
          ...
          await auth.signInWithCredential(credential).then((value) {
            final String str = value.user!.email.toString().replaceAll(".", "1");
            final String node = str.substring(0, str.indexOf('@'));
            database.ref('Accounts').child(node).set({
